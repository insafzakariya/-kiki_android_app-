...
import lk.mobilevisions.kiki.audio.adapter.LibraryHomeSongListAdapter;
...
public class LibraryHomeSongListFragment extends Fragment implements LibraryHomeSongListAdapter.OnLibraryHomeSongItemClickListener, AddSongToPlaylistDialogAdapter.OnPlaylistDialogItemClickListener {
    @Inject
    TvManager tvManager;

    FragmentGenreWiseSongsBinding binding;

    LibraryHomeSongListAdapter mAdapter;
    private List<Song> librarySongList = new ArrayList<>();
    ...
    public LibraryHomeSongListFragment() {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mAdapter = new LibraryHomeSongListAdapter(getActivity(), librarySongList, LibraryHomeSongListFragment.this);
    ...
    private void getLibrarySongs() {
        ...
        tvManager.getLibrarySongs(100,1, new APIListener<List<Song>>() {
            @Override
            public void onSuccess(List<Song> result, List<Object> params) {
                librarySongList = result;
                binding.genreWiseRecyclerview.setAdapter(new LibraryHomeSongListAdapter(getContext(),
                        librarySongList, LibraryHomeSongListFragment.this));
                if (result.size() <= 0) {
                    binding.genreWiseRecyclerview.setVisibility(View.GONE);
                    binding.aviProgress.setVisibility(View.GONE);

                } else {

                    binding.genreWiseRecyclerview.setVisibility(View.VISIBLE);
                    binding.aviProgress.setVisibility(View.GONE);
                }
    ...
    private void addToLibraryPlaylistDialog(List<PlayList> playLists) {
        ...
        AddSongToPlaylistDialogAdapter adapter = new AddSongToPlaylistDialogAdapter(getActivity(),playLists,LibraryHomeSongListFragment.this);
    ...
    private void removeFromLibraryDialog(Song song) {
...
//        songTitle.setText(song.getName());
    ...
    private void removeDatafromLibrary(int songId){
        ArrayList <Integer> removeSongId = new ArrayList<>();
        removeSongId.add(songId);
        System.out.println("sjdkbfsdjkbv " + removeSongId);
        tvManager.removeDatafromLibrary("S",removeSongId, new APIListener<Void>() {
            @Override
            public void onSuccess(Void result, List<Object> params) {
...
//                mAdapter.notifyDataSetChanged();
    ...
    public void onRemoveHomeSongItemClick(Song song, int position, List<Song> songs) {
        removeFromLibraryDialog(song);
    }

//    @Override
//    public void onRemoveHomeSongItemClick(Song song) {
//
//
//
//
