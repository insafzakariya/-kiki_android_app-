...
public class ChatManager {
    ...
    public void getAvatarImages(final APIListener<List<Avatar>> listener) {

        api.getAvatarImages(Utils.Auth.getBasicAuthToken(application), Utils.Auth.getBearerToken(application)).enqueue(new Callback<List<Avatar>>() {
    ...
    public void uploadUserImage(String image, String type, int id, final APIListener<Void> listener) {
        HashMap<String, Object> request = new HashMap<>();
        request.put("image", image);
        request.put("type", type);
        request.put("avatar_id", id);

        api.uploadUserImage(Utils.Auth.getBasicAuthToken(application), Utils.Auth.getBearerToken(application),"application/json", request)
                .enqueue(new Callback<Void>() {

                    @Override
                    public void onResponse(Call<Void> call, Response<Void> response) {
                        System.out.println("jdhfhsdf 555  " + response.code());
                        switch (response.code()) {
                            case 201:
                                listener.onSuccess(null, null);
                                break;
